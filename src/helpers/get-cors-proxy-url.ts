import { Abortable, asyncFetchJSON, AsyncTask, IAsyncFetchRequestInit, IAsyncTaskConstraint, asyncFetch } from '@lirx/async-task';
...
export function asyncFetchWithoutCors(
  input: RequestInfo | URL,
  init: IAsyncFetchRequestInit,
  abortable: Abortable,
): AsyncTask<Response> {
  return forgeNoCorsRequest(input, init, abortable)
    .successful((request: Request, abortable: Abortable): AsyncTask<Response> => {
      return asyncFetch(
        request,
        init,
        abortable,
